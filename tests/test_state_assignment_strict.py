...
def main():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        s.x = 1.9
        assert any(
            "Type mismatch" in str(x.message) for x in w
        ), "expected a dtype mismatch warning"
        assert s.x == 1.9
    ...
    with warnings.catch_warnings(record=True) as w2:
        ...
        assert any(
            "Type mismatch" in str(x.message) for x in w2
        ), "expected dtype mismatch warning on bulk assign"
    ...
    with warnings.catch_warnings(record=True) as w3:
        ...
        s2.X = np.array([1, 2, 3])
        assert any(
            "Type mismatch" in str(x.message) for x in w3
        ), "expected dtype mismatch warning when assigning ints to float state"
