...
When the coefficients of the system of linear differential (or difference) equations are constant, the
state model represents a linear system (LTI, linear time invariant).
If the coefficients are
time-varying, the system is considered linear time varying.
...
- :math:`\omega` is the (Gaussian) process uncertainty with covariance matrix :math:`Q_c`
...
- :math:`\nu` is the (Gaussian) measure noise with covariance matrix :math:`R_c`
...
in a linear Kalman provides the error in the state estimates.
...
- :math:`\omega` is the (Gaussian) process uncertainty
...
Finally the instantaneous linear model of system :eq:`_nonlinearmodel` is:
...
The Kalman gain, :math:`K`, is computed based on the predicted state error covariance and the measurement noise covariance.
...
- :math:`\omega` is the (Gaussian) process uncertainty with covariance matrix :math:`Q`
- :math:`y` is the system output vector
- :math:`h(\cdot)` is an arbitrary vector-valued function representing the system output
- :math:`\nu` is the (Gaussian) measure noise with covariance matrix :math:`R`
- :math:`x_0` is a vector of initial conditions
