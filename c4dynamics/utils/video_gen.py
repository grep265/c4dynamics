...
def animate(
    ...
    rotate=False,
    pathangle=None,
    motion=Motion_Type.LINEAR,
    saveshow=Im_saveshow.SAVESHOW.value,
):
    ...
    im_object = cv2.resize(
        im_object, tuple(reversed(tuple(int(scaling * i) for i in im_object.shape[:2])))
    )
    ...
    x_velocity = int(
        vtotal_diag * np.cos(pathangle * np.pi / 180)
    )  # int(FRAMEWIDTH / 30)  # Adjust velocity based on frame width
    y_velocity = int(
        vtotal_diag * np.sin(pathangle * np.pi / 180)
    )  #  int(FRAMEHEIGHT / 30)  # Adjust velocity based on frame width
...
for runcase in range(1, 6):
    ...
    if runcase == 1:
        ...
        animate(objpath=objpath, motion=Motion_Type.PARABOLE, x0=0, y0=0, scaling=0.5)

    elif runcase == 5:
        """sinusoid 1: left to right"""
        animate(objpath=objpath, motion=Motion_Type.SINE, x0=0, y0=FRAMEHEIGHT / 2, scaling=0.5)
