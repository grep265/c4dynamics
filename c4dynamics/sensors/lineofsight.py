...
class lineofsight:
    ...
    def measure(obj, r, v):
        ...
        ##
        obj.omega_ach = obj.omega_ach * np.exp(-obj.dt / obj.tau1) + obj.omega * (
            1 - np.exp(-obj.dt / obj.tau1)
        )  # lag

        #
        # final processed tracking rate signal vector
        ##
        obj.omega_f = obj.omega_f * np.exp(-obj.dt / obj.tau2) + obj.omega_ach * (
            1 - np.exp(-obj.dt / obj.tau2)
        )  # filter
    ...
    def store(obj, t=-1):
        obj._data = np.vstack((obj._data, np.concatenate(([t], obj.omega, obj.omega_f)))).copy()
