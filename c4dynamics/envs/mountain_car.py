...
class mountain_car(c4d.state):
    ...
    **Discretization**
    ...
    def __init__(self, mode="physical", n_bins=12):
        ...
        self.state_lim = (-1, 1)
        self.state_interval = np.linspace(
            self.state_lim[0], self.state_lim[1], num=self.n_bins - 1, endpoint=False
        )

        gamma = 3.0

        uniform_interval = np.linspace(-1, 1, num=self.n_bins)
        gamma_interval = np.sign(uniform_interval) * (
            np.abs(uniform_interval) ** gamma
        )  # flatten center

        self.x_interval = self.x_lim[0] + (gamma_interval + 1) * 0.5 * (
            self.x_lim[1] - self.x_lim[0]
        )
        self.vel_interval = self.speed_lim[0] + (gamma_interval + 1) * 0.5 * (
            self.speed_lim[1] - self.speed_lim[0]
        )

        xp = []
        for x in np.linspace(self.x_lim[0], self.x_lim[1], 5000):
            xp.append(
                [x, integrate.quad(lambda xx: np.sqrt(1 + self.dhdx(xx) ** 2), self.x_lim[0], x)[0]]
            )
        self.xp = np.array(xp)
    ...
    def step(self, action):
        ...
        # in t-n:
        acc = (
            self.action_list[action] / self.mass
    ...
    def energy_normalized(self, state=None):
        ...
        e_state = (
            self.mass * self.gravity * self.hx(self.p2x(state[0])) + 0.5 * self.mass * state[1] ** 2
        )
    ...
    def energy(self, state=None):
        ...
        e_state = (
            self.mass * self.gravity * (np.sin((state[0] - (0.5 * np.pi)) + 0.5) + 1.0)
            + 0.5 * state[1] ** 2
        )
    ...
    def discretize(self):
        ...
        xi = self.p2x(
            self.position
        )  # NOTE it can be left in p to save time but change intervals as well
    ...
    def test_energy_balance(self, x0, v0):
        ...
        PE_start = (
            self.mass * self.gravity * (1 + self.hx(x0))
        )  # add a baseline of 1 to avoid negative PE)
        ...
        W_engine = self.action_list[2] * (
            self.x2p(self.x_lim[1]) - self.x2p(x0)
        )  # (self.x_lim[1] - x) # * np.cos(theta) # maybe we shouldnt project the path on x because the force is integrated along the entire path.
...
def plot_curve(
    ...
    kernel = np.ones(int(kernel_size)) / float(
        kernel_size
    )  # Smooth the graph using a convolution. since it sums kernel_size items, it also have to divide by the same size to make average.
    ...
    #         94,         250     -156                       94.
    ax.plot(
        np.arange(data_length)[lower_boundary:upper_boundary],
        data_convolved_array,
        color,
        alpha=1.0,
    )  # Convolved plot
