...
def ideal(tgt):
    ...
    Xb = np.array(
        [
            skr_ideal.BR @ [X[1] - skr_ideal.x, X[2] - skr_ideal.y, X[3] - skr_ideal.z]
            for X in tgt.data()
        ]
    )
...
def yawing(tgt, skr_ideal):
    ...
    axs[0].plot(
        *skr_ideal.data("az", c4d.r2d), "m", linewidth=1, label="ideal static seeker", zorder=12
    )
    axs[0].plot(
        *skr.data("az", c4d.r2d), "c", linewidth=1, label="non-ideal yawing seeker", zorder=11
    )
    c4d.plotdefaults(axs[0], "", "", "Azimuth (deg)", 8)
    axs[0].legend(fontsize=4, facecolor=None)
...
def bias1(tgt):
    ...
    for i, x in enumerate(tgt.data()):
        ...
        if np.abs(el1 - el2) < 0.01 * c4d.d2r or np.abs(az1 - az2) < 0.01 * c4d.d2r:
            print(
                f"{i} t = {x[0]} :    del {np.abs(el1 - el2) * c4d.r2d}   daz {np.abs(az1 - az2) * c4d.r2d}"
            )
...
def bias2():
    ...
    ax.hist(
        seekers_type_B, 30, facecolor="cyan", edgecolor="black", label="Type B", zorder=11, alpha=1
    )
