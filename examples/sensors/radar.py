...
def intro():
    ...
    altmtr = c4d.sensors.radar(rng_noise_std=0.5, dt=dtsensor)
    ...
    fig, ax = plt.subplots(
        ...
        gridspec_kw={"left": 0.15, "right": 0.9, "hspace": 0.5, "top": 0.9, "bottom": 0.2},
    )
    ...
    ax.legend(
        fontsize="xx-small", facecolor=None, framealpha=0.5
    )  # , edgecolor = None Set font size and legend box properties
...
def target():
    ...
    plt.savefig(c4d.j(savedir, "target.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def ideal(tgt):
    ...
    Xb = np.array(
        [
            rdr_ideal.BR @ [X[1] - rdr_ideal.x, X[2] - rdr_ideal.y, X[3] - rdr_ideal.z]
            for X in tgt.data()
        ]
    )
    ...
    axs[1].plot(
        tgt.data("t"), c4d.atan2d(Xb[:, 1], Xb[:, 0]), "m", label="target azimuth", linewidth=2
    )
    axs[1].plot(*rdr_ideal.data("az", scale=c4d.r2d), "c", label="radar azimuth", linewidth=1)

    axs[1].plot(
        tgt.data("t"),
        c4d.atan2d(Xb[:, 2], c4d.sqrt(Xb[:, 0] ** 2 + Xb[:, 1] ** 2)),
        "r",
    ...
    plt.savefig(c4d.j(savedir, "ideal.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def nonideal(tgt, rdr_ideal):
    ...
    axs[1].plot(
        *rdr_ideal.data("az", scale=c4d.r2d), "m", linewidth=1, label="target azimuth", zorder=12
    )
    axs[1].plot(*rdr.data("az", scale=c4d.r2d), "c", linewidth=1, label="radar azimuth", zorder=11)

    axs[1].plot(
        *rdr_ideal.data("el", scale=c4d.r2d), "r", linewidth=1, zorder=12, label="target elevation"
    )
    axs[1].plot(
        *rdr.data("el", scale=c4d.r2d), "y", linewidth=1, zorder=11, label="radar elevation"
    )

    axs[1].legend(fontsize=4, facecolor=None)
    c4d.plotdefaults(axs[1], "", "Time (s)", "Angles (deg)", 8)

    plt.savefig(c4d.j(savedir, "nonideal.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def yawing(tgt, rdr_ideal):
    ...
    axs[1].plot(
        *rdr_ideal.data("az", c4d.r2d), "m", linewidth=1, label="az: ideal static", zorder=12
    )
    axs[1].plot(*rdr.data("az", c4d.r2d), "c", linewidth=1, label="az: non-ideal yawing", zorder=11)

    axs[1].plot(
        *rdr_ideal.data("el", scale=c4d.r2d), "r", label="el: ideal static", linewidth=1, zorder=12
    )
    axs[1].plot(
        *rdr.data("el", scale=c4d.r2d), "y", label="el: non-ideal yawing", linewidth=1, zorder=11
    )

    axs[1].legend(fontsize=4, facecolor=None, loc="upper left")
    c4d.plotdefaults(axs[1], "", "Time (s)", "Angles (deg)", 8)

    plt.savefig(c4d.j(savedir, "yawing.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def timestep(tgt):
    ...
    for t in np.arange(0, 0.025, 0.005):
    ...
    ax.hist(
        seekers, 30, facecolor="magenta", edgecolor="black", label="Seekers", zorder=12, alpha=1
    )
    ...
    plt.savefig(c4d.j(savedir, "bias2.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def measure(tgt):
    ...
    axs[1].legend(fontsize=4, facecolor=None)
    plt.savefig(c4d.j(savedir, "measure.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
    ...
    axs[1].legend(fontsize=4, facecolor=None)
    plt.savefig(c4d.j(savedir, "measure_zoom.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def bias1(tgt):
    ...
    plt.savefig(c4d.j(savedir, "bias1.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
...
def SF(tgt):
    ...
    plt.savefig(c4d.j(savedir, "sf.png"), bbox_inches="tight", pad_inches=0.2, dpi=600)
