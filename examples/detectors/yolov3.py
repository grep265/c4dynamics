...
def intro():
    ...
    plt.savefig(c4d.j(savedir, "intro.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
...
def nms():
    ...
    plt.savefig(c4d.j(savedir, "nms_th.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
...
def confidence_th():
    ...
    plt.savefig(c4d.j(savedir, "confidence_th.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
...
def singleimage():
    ...
    plt.axis(False)
    plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    plt.savefig(c4d.j(savedir, "single_image.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
...
def outforamt():
    ...
    for i, p in enumerate(pts):
        print(
            "{:^10d} | {:^10.3f} | {:^10.3f} | {:^16} | {:^16} | {:^10} | {:^14}".format(
                i, p.x, p.y, ptup(p.box[0]), ptup(p.box[1]), p.class_id, ptup(p.fsize)
            )
        )
        cv2.rectangle(img, p.box[0], p.box[1], [0, 255, 0], 2)
        point = (int((p.box[0][0] + p.box[1][0]) / 2 - 75), p.box[1][1] + 22)
    ...
    plt.axis(False)
    plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    plt.savefig(c4d.j(savedir, "outformat.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
