...
def scaling():
    ...
    plt.savefig(
        os.path.join(savedir, "plot_scale.png"), bbox_inches="tight", pad_inches=0.2, dpi=600
    )
    return s
...
def anotheraxis(s):
    ...
    plt.savefig(
        os.path.join(savedir, "plot_axis.png"), bbox_inches="tight", pad_inches=0.2, dpi=600
    )
...
def sideview():
    ...
    for t in np.arange(0, 10, dt):
        floating_balloon.inteqm(forces=[0, 0, 0.05], dt=dt)
    ...
    plt.savefig(
        os.path.join(savedir, "plot_dp_inteqm3.png"), bbox_inches="tight", pad_inches=0.2, dpi=600
    )
