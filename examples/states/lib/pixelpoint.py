...
def detect_triangle():
    ...
    for tri in triangles:
        pp = c4d.pixelpoint(
            x=int(tri[0] + tri[2] / 2), y=int(tri[1] + tri[3] / 2), w=tri[2], h=tri[3]
        )

        pp.fsize = img.shape[:2]
        pp.class_id = "triangle"
        print(
            "{:^10.1f} | {:^10.1f} | {:^16} | {:^16} | {:^10} | {:^14}".format(
                pp.x, pp.y, ptup(pp.box[0]), ptup(pp.box[1]), pp.class_id, ptup(pp.fsize)
            )
        )
    ...
    plt.savefig(c4d.j(savedir, "triangle.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
...
def yolo():
    ...
    plt.axis(False)
    plt.imshow(cv2.cvtColor(img, cv2.COLOR_BGR2RGB))
    plt.savefig(c4d.j(savedir, "yolov3.png"), bbox_inches="tight", pad_inches=0.05, dpi=600)
