...
def draw_kf(ekf, title, trueobj=None, filename=None, measures=True, std=False):
    ...
    _, ax = c4d._figdef()
    ax.plot(*ekf.data("K"), "m", linewidth=0.8)
...
def varying_Q(cont=False):
    ...
    ekf = c4d.filters.ekf({"x": 0.0}, P0=500**2, F=1, G=v, H=1, Q=0, R=sensor_noise**2)
    ekf.detect = 0
    ekf.K = 0
...
def varying_R():
    ...
    ekf = c4d.filters.ekf({"x": 0.0}, P0=0.5**2, F=1, G=v, H=1, Q=0.05)
