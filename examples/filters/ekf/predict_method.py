...
def plain_predict():
    ...
    c4d.cprint("plain predict", "y")

    ekf = c4d.filters.ekf({"x": 0}, P0=0.5**2, F=1, H=1, Q=0.05, R=200)
    print(ekf)
    print(ekf.X)
    print(ekf.P)
    ekf.predict()
...
def with_input(cont=False):

    c4d.cprint("with input", "y")

    ekf = c4d.filters.ekf({"x": 0}, P0=0.5**2, F=1, G=150, H=1, Q=0.05, R=200)
    print(ekf.X)
    print(ekf.P)
    ekf.predict(u=1)
    print(ekf.X)
...
def varying_Q():
    ...
    c4d.cprint("varying Q", "y")

    ekf = c4d.filters.ekf({"x": 0}, P0=0.5**2, F=1, G=150, H=1, Q=0.05, R=200)

    print(ekf.X)
    print(ekf.P)
    ekf.predict(u=1, Q=0.01)
    print(ekf.X)
    print(ekf.P)
