...
def drawekf(ekf=None, trueobj=None, measures=None, title="", filename=None, std=False):
    ...
    if trueobj:
        ax[2].plot(
            *trueobj.data("beta"), "m", linewidth=1.2, label="true"
        )  # label = r'$\gamma$') #'\\gamma') #
    ...
    if filename:
        plt.savefig(c4d.j(savedir, filename + ".png"), pad_inches=0.2, dpi=600)
...
def filtered():
    ...
    p0 = np.diag(
        [nu**2, vzerr**2, betaerr**2]
    )  # when a list is given the std are provided. when matrix the variances.
    R = nu**2 / dt
    ...
    H = [1, 0, 0]
    ekf = c4d.filters.ekf(
        X={"z": tgt.z + zerr, "vz": tgt.vz + vzerr, "beta": tgt.beta + betaerr},
        P0=p0,
        H=H,
        Q=Q,
        R=R,
