...
w1 = 0.5  # ft. - 1std; np.sqrt(.5) # w1 = 0.3, 1std; Q11 = 0.1, variance.
w2 = (
    0.1 * c4d.d2r
)  # 0 # .1 * c4d.d2r # deg. - 1std; np.sqrt(.3 * c4d.d2r) # w2 = 1, 1std; Q22 = 1 variance.
...
c4d.cprint(
    (
        f"The system is observable (rank = n = {n})."
        if rank == n
        else "The system is not observable (rank = {rank), n = {n})."
    ),
    "y",
)
...
def drawkf(kf=None, trueobj=None, measures=None, title="", filename=None, std=False):
    ...
    if std:
        ...
        # ±std
        ax[0].plot(
            t_sig, x + np.sqrt(x_sig.squeeze()), linewidth=1, color="w", label="std"
        )  # np.array(v.color) / 255)
        ax[0].plot(
            t_sig, x - np.sqrt(x_sig.squeeze()), linewidth=1, color="w"
        )  # np.array(v.color) / 255)
        # correct
    ...
    if trueobj:
        ax[1].plot(
            *trueobj.data("gamma", c4d.r2d), "m", linewidth=1.2, label="true"
        )  # label = r'$\gamma$') #'\\gamma') #
    ...
    if std:
        ...
        # ±std
        ax[1].plot(
            t_sig, (x + np.sqrt(x_sig.squeeze())) * c4d.r2d, linewidth=1, color="w", label="std"
        )  # np.array(v.color) / 255)
        ax[1].plot(
            t_sig, (x - np.sqrt(x_sig.squeeze())) * c4d.r2d, linewidth=1, color="w"
        )  # np.array(v.color) / 255)
        # state
...
def ideal():
    ...
    for t in tspan:
        ...
        if Z is not None:
            tgt.X = odeint(
                autopilot, tgt.X, [t, t + dt], args=(Hf - Z, process_noise @ np.random.randn(2))
            )[-1]
...
def filtered():
    ...
    Qk = np.array(
        [[0.250000002538478, 7.59005070968549e-8], [7.59005070968549e-8, 3.03096871166273e-6]]
    )
    ...
    kf = c4d.filters.kalman(
        X={"z": tgt.z + z_err, "gamma": tgt.gamma + gma_err},
    ...
    for t in tspan:
        ...
        tgt.X = solve_ivp(
            autopilot_ivp, [t, t + dt], tgt.X, args=(Hf - kf.z, process_noise), method="BDF"
        ).y[:, -1]
