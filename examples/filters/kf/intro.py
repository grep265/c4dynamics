...
def draw_kf(kf, title, trueobj=None, filename=None, measures=True, std=False):
    ...
    if trueobj:
        ...
        ax.get_lines()[0].set_linewidth(0.5)
    ...
    if filename:
        plt.savefig(c4d.j(savedir, filename + ".png"), bbox_inches="tight", pad_inches=0.2, dpi=600)

    _, ax = c4d._figdef()
    ax.plot(*kf.data("K"), "m", linewidth=0.8)
...
def steadystate(cont=False):
    ...
    # else:
    kf = kalman({"x": 0.0}, P0=0.5**2, F=1, G=v, H=1, Q=0.05, R=sensor_noise**2, steadystate=True)

    train = c4d.state(x=0.0)
    train.x = 0
...
def varying_Q(cont=False):
    ...
    kf = kalman({"x": 0.0}, P0=500**2, F=1, G=v, H=1, Q=0, R=sensor_noise**2)
    kf.detect = 0
    kf.K = 0

    for t in range(1, 25 + 1):  #  seconds.

        process_noise = 0.1 * t  # (1 + t)#(1 + 50 * t * np.random.randn() / v)
...
def varying_R():
    ...
    kf = kalman({"x": 0.0}, P0=0.5**2, F=1, G=v, H=1, Q=0.05)
